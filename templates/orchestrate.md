...
Optional split for large repos: move worker/reviewer-facing sections (identity, verify gate, review gate, PR conventions, code rules) into a **worker-scoped conventions file** (e.g. `.claude/worker-conventions.md`) and declare it here — worker contracts then name that file as their read order and workers skip this orchestrator file entirely.
...
- One small concern per PR; draft vs ready: `<ready-for-review | draft-first (orchestrator flips ready after final check)>`.
