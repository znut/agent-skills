...
function ghJson(args) {
	// env passed explicitly: Bun's execFileSync ignores process.env mutations
	// made after start, and ensureAuth() sets GH_TOKEN at runtime.
...
// Issue-side search instead of a projectV2 items() sweep: items() has no
// server-side filter and returns oldest-first, so a whole-board pull grows
// with every closed ticket and a capped sweep drops the newest rows. Two
// searches, open plus closed within retention, cost proportional to the live
// set. Issues not on the target board are skipped. GitHub caps search at
// 1000 results per query; warned on, not handled.
