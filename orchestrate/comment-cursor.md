# Stakeholder-comment cursor (when the repo rules declare one)
...
- (a) At boot, read your named session's returned `paths.comment_cursor` (sessions without named boot keep `<role>.json`; field `last_seen`, ISO-8601 UTC) and fetch ALL stakeholder comments since that stamp with the tracker's repo-wide "since" queries — for GitHub, TWO calls total: `issues/comments?since=` AND `pulls/comments?since=` (review-thread comments are a separate endpoint the CLI's PR view misses); never a per-PR loop.
...
- (e) Mid-session the cursor is blind between sweeps — if the repo's PR-status service materializes comment events as files, arm a background watcher on those events too (sweep-on-fire, re-arm after).
- (f) Approval/review BODIES are a third comment surface the two since= endpoints miss — on an approved/merged event for a lane PR, also read that PR's reviews (`pulls/<n>/reviews`) for stakeholder notes inside the approval. Where the service emits a UNIFIED per-PR timeline log, arm ONE watcher over that log (with a local actor filter), never per-event-type marker loops.
