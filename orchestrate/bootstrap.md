...
Write the answers into a generated `.claude/orchestrate.md` (from this skill repo's `templates/orchestrate.md` — including the `## Enforcement policy` section reflecting answers 2 and 4; the identity/marker guards default to ENFORCE so opt-outs must be written explicitly, while the opt-in `draft_first`/`label_check` slots are kept only if the user wants them) plus `.claude/agents/` from `templates/agents/`, land them via the normal pipeline, and proceed. Never re-interview while the file exists — edit the file instead.
