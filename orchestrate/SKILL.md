...
8. **Return** the PR URL + a short summary. Done.
...
| Case | Model |
|---|---|
| Implementation, edits, UI, tests, configs | default/`sonnet`-tier |
| Architecture decision, multi-file refactor, security-sensitive, tricky bug | strongest available |
| "Where is X / what calls Y / list uses" (read-only locator) | `Explore` subagent |
| Worker re-dispatched after first failure | escalate one tier |

Orchestrator decides escalation — do not ask user unless cost is extreme.

**Model is ALWAYS explicit, at every level**: the orchestrator passes `model: "sonnet"` on every worker dispatch (escalate deliberately, never by accident), and worker prompts instruct that any subagent the worker itself spawns (reviewers, locators) also passes an explicit model — an omitted model inherits the TOP session's model, not the spawning worker's, which silently runs helpers on the most expensive tier.
...
- Worktree branches off latest default branch (harness creates at dispatch time with `isolation: "worktree"`).
- Harness auto-removes unchanged worktrees.
- After PR opened, list remaining worktrees: `git worktree list`. Clean manually if stale: `git worktree remove <path>`.
- If a branch needs rebase after PR: `git -C <worktree> rebase origin/<default>`, re-run verify, re-push.
